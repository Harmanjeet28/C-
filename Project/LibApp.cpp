...
namespace sdds {
	...
	void LibApp::returnPub() {
		...
		if (result) {
			//If the user selects a publication and confirms the return using the prompt:
			bool select = confirm("Return Publication?");
			...
			if (select) {
				...
				if ((Date() - getPub(result)->checkoutDate()) > SDDS_MAX_LOAN_DAYS) {
					double pay = ((Date() - getPub(result)->checkoutDate()) - SDDS_MAX_LOAN_DAYS) * 0.5;
					...
					cout << "Please pay $" << fixed << setprecision(2)<<pay << " penalty for being " << ((Date() - getPub(result)->checkoutDate()) - SDDS_MAX_LOAN_DAYS) << " days late!" << endl;
				}
