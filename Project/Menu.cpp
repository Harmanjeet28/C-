...
namespace sdds {
    ...
    void Menu::setMenuTitle(const char* title) {
        // Allocate memory for the title content
        menuTitle.itemContent = new char[strlen(title) + 1];

        // Copy the title content to the allocated memory
        strcpy(menuTitle.itemContent, title);
    }


    void Menu::changeName(const char* title) {
        if (title) {
            setMenuTitle(title);
        }
    }
